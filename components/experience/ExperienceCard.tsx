...
export default function ExperienceCard({ experience }: Props) {
  ...
  // Safety check - if experience is undefined or null, return null
  if (!experience) {
    return null;
  }
  ...
  const isFullPlace = (place: any): place is Place => {
    return place && typeof place === 'object' && 'description' in place && 'coverVideo' in place;
  };
  ...
  const getPlaceName = () => {
    ...
    if (experience.place) return experience.place.name;
    return '';
  };

  const getPlaceId = () => {
    if (experience.place) return experience.place.id;
    return experience.placeId || '';
  };
  ...
  const formatPrice = () => {
    ...
    const symbol = price.currency === 'USD' ? '$' : price.currency === 'EUR' ? '€' : 'Frw';
    ...
    if (price.type === 'range') {
      return `${prefix}${symbol}${price.min} - ${symbol}${price.max}`;
    }
    
    return `${prefix}${symbol}${price.amount}${price.period ? `/${price.period}` : ''}`;
  };
    ...
    <>
      <div className="relative h-screen w-full overflow-hidden">
        {video ? (
          ...
          <div className="absolute h-full w-full bg-gradient-to-b from-zinc-800 to-black flex items-center justify-center">
        ...
        <div className="absolute bottom-28 left-6 right-20 z-20">
          <p className="text-gold font-bold">{experience.category || 'Experience'}</p>
          
          {/* Title with Save Button inline */}
          <div className="flex items-center gap-3 mt-1">
            <h2 className="text-3xl font-bold">{experience.title || 'Untitled'}</h2>
            <button
          ...
          <p className="text-white/50 text-sm mt-1 max-w-md line-clamp-2">{experience.description || ''}</p>
        ...
        <div className="fixed right-5 bottom-32 z-40 flex flex-col gap-4">
          {!isVideoMemory ? (
            <>
              {/* Phone Button */}
              <button className="h-12 w-12 rounded-full bg-gold flex items-center justify-center text-black shadow-xl hover:bg-gold/80 transition hover:scale-105">
              ...
              {/* WhatsApp Button */}
              <button className="h-12 w-12 rounded-full bg-[#25D366] flex items-center justify-center text-white shadow-xl hover:opacity-80 transition hover:scale-105">
